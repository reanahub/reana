...
def _get_prs_from_issue(repo, issue_number):
    """Return sorted (component, pr_number) pairs for PRs linked to an issue."""
    output = run_command(
        [
            "gh",
            "issue",
    ...
    if not prs:
        click.secho(
            f"No PRs currently linked to reanahub/{repo}#{issue_number}.",
            fg="yellow",
        )
...
@click.option(
    ...
    nargs=2,
    default=None,
    help="Derive PRs currently linked to a GitHub issue. [repo issue#]",
)
@click.option(
    "--fetch",
    is_flag=True,
...
def git_checkout_pr(branch, issue, fetch, pull, reset):  # noqa: D301
    ...
    derive the set of PRs currently linked to a GitHub issue instead.
    ...
    :param issue: Derive PRs to check out from the pull requests currently
                  linked to a GitHub issue. The value consists of two strings:
                  the reanahub repository name and the issue number. For
                  example, ``-i reana-commons 123`` will check out all PRs that
