...
(
    ...
    zip -0 -q validation-bundle.zip reana.yaml Snakefile
)
...
policy_selector=$(kubectl -n "${kubernetes_namespace}" get networkpolicy \
    "${policy_name}" \
    -o go-template='{{len .spec.podSelector.matchLabels}}|{{index .spec.podSelector.matchLabels "app"}}|{{index .spec.podSelector.matchLabels "reana_workflow_mode"}}|{{index .spec.podSelector.matchLabels "app.kubernetes.io/instance"}}')
expected_policy_selector="3|reana-run-validation|spec-validation|${instance_name}"
if [ "${policy_selector}" != "${expected_policy_selector}" ]; then
    echo "Error: NetworkPolicy '${policy_name}' has an unexpected pod selector." >&2
    echo "Expected: ${expected_policy_selector}" >&2
    echo "Actual:   ${policy_selector}" >&2
    exit 1
fi
...
if ! kubectl -n "${kubernetes_namespace}" get pod \
    -l "app=reana-run-validation,reana_workflow_mode=spec-validation,app.kubernetes.io/instance=${instance_name}" \
    -o name | grep -Fxq "pod/${probe_pod_name}"; then
    echo "Error: NetworkPolicy selector does not select probe pod '${probe_pod_name}'." >&2
    exit 1
fi
