...
get_multikueue_cluster_name() {
    local remote="$1"
    echo "$remote-multikueue-cluster"
}
...
metadata:
  namespace: "default"
  name: "$REMOTE-job-queue"
spec:
  clusterQueue: "$REMOTE-job-queue"
EOF
    ...
    if [[ "$CLUSTER_MODE" = "local" ]]; then
        # Only the manager cluster specifies admission checks in the job ClusterQueue
        admission_checks="  admissionChecks:\n  - $REMOTE-job-admission-check"
    fi
...
kind: AdmissionCheck
metadata:
  name: $REMOTE-job-admission-check
spec:
...
get_workflow_id() {
    ...
    if [[ -z "$workflow_id" ]]; then
        print_error "No workflows found with name '$WORKFLOW_NAME'. Please run 'reana-client create -w test' in your workflow directory first." >&2
        return 1
    fi
...
get_workflow_dir() {
    ...
    echo "/var/reana/users/00000000-0000-0000-0000-000000000000/workflows/$workflow_id"
}
...
copy_between_pods() {
    ...
    while true; do
        if kubectl_with "$src_machine" exec "$src_pod" -- test -e "$src_path" 2>/dev/null; then
            echo "Path '$src_path' found on $src_machine."
            break
        else
            ...
            echo "Waiting for path at '$src_machine:$src_pod:$src_path'..."
            sleep 2
        fi
    done
...
usage() {
    ...
    *)
        ...
        echo "  $0 copy datacenter1:results/plot.png local:results/plot.png"
...
connect)
    if [[ "$CLUSTER_MODE" = "local" ]]; then
        print_error "Please specify a remote cluster to connect using --remote <remote>."
        exit 1
    else
        connect_remote
    fi
